<template>
    <div class="w-[20%] bg-white py-8 flex flex-col items-center gap-10 shadow-xl fixed right-0 overflow-y-scroll z-10 h-full transition-all"
        ...
        <RouterLink to="/hajjpage-digitalid" class="w-full text-lg font-bold text-site-text-grey relative flex items-center justify-center">
            <div v-if="useRoute().path.includes('hajjpage-digitalid')" class="w-[95%] h-14 left-0 bg-site-blue absolute top-1/2 -translate-y-1/2" 
                style="border-top-right-radius: 8rem;border-bottom-right-radius: 8rem;"></div>
            <span 
            class="z-10 relative"
            :class="[useRoute().path.includes('hajjpage-digitalid') ? 'text-white' : '' , sidebarHidden ? 'invisible' : '']">الهوية الرقمية</span>
        </RouterLink>
        <RouterLink to="/hajjpage-notifications" class="w-full text-lg font-bold text-site-text-grey relative flex items-center justify-center">
            <div v-if="useRoute().path.includes('hajjpage-notifications')" class="w-[95%] h-14 left-0 bg-site-blue absolute top-1/2 -translate-y-1/2" 
                style="border-top-right-radius: 8rem;border-bottom-right-radius: 8rem;"></div>
            <span 
            class="z-10 relative"
            :class="[useRoute().path.includes('hajjpage-notifications') ? 'text-white' : '' , sidebarHidden ? 'invisible' : '']">الإشعارات والعمليات</span>
        </RouterLink>
        <RouterLink to="/hajjpage-messages" class="w-full text-lg font-bold text-site-text-grey relative flex items-center justify-center">
            <div v-if="( useRoute().path.includes('hajjpage-messages') || useRoute().path.includes('hajjpage-recieved-messages') )" class="w-[75%] h-14 left-0 bg-site-blue absolute top-1/2 -translate-y-1/2" 
                style="border-top-right-radius: 8rem;border-bottom-right-radius: 8rem;"></div>
            <span 
            class="z-10 relative"
            :class="[ ( useRoute().path.includes('hajjpage-messages') || useRoute().path.includes('hajjpage-recieved-messages') ) ? 'text-white' : '' , sidebarHidden ? 'invisible' : '']">الرسائل</span>
        </RouterLink>
        <RouterLink to="/hajjpage-attachment" class="w-full text-lg font-bold text-site-text-grey relative flex items-center justify-center">
            <div v-if="useRoute().path.includes('hajjpage-attachment')" class="w-[75%] h-14 left-0 bg-site-blue absolute top-1/2 -translate-y-1/2" 
                style="border-top-right-radius: 8rem;border-bottom-right-radius: 8rem;"></div>
            <span 
            class="z-10 relative"
            :class="[useRoute().path.includes('hajjpage-attachment') ? 'text-white' : '' , sidebarHidden ? 'invisible' : '']">المرفقات</span>
        </RouterLink>
